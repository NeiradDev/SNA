...
<style>
  ...
  .kpi-title{font-size:12px;color:#666;margin:0 0 6px 0;}
  .kpi-value{font-size:34px;font-weight:800;line-height:1;margin:0;}
  ...
  .month-filter button:hover{background:#f2f2f2;}
</style>
<?php endif; ?>
...
(() => {
  ...
  const wednesdaysInMonth = (y, m) => {
    ...
    const arr = [];
    while (cur <= last) { arr.push(new Date(cur)); cur = addDays(cur, 7); }
    return arr;
  };
  ...
  const update = () => {
    ...
    const { y, m } = parsed;
    const { labels, values } = getDataDemo(y, m);
