            ...
            <div class="row g-4">
              <div class="col-md-3">
                ...
                <div class="date-help mt-2">
                  <?= $isEdit ? 'Fecha inicio fija (no editable).' : 'Puedes escoger días anteriores, actuales o futuros.' ?>
                </div>
              </div>
              ...
              <div class="col-md-3">
                ...
                <div class="date-help mt-2">
                  ...
                  <?php else: ?>
                    Debe ser igual o mayor a inicio.
                  <?php endif; ?>
                </div>
              </div>
                ...
                <div id="recurrence_panel" class="mt-3" style="display:none;">
                  <div class="row g-3">
                    ...
                    <div class="col-md-4">
                      ...
                      <div class="text-muted small mt-1">Se permiten días anteriores.</div>
                    </div>
...
<script>
  ...
  /* =========================================================
     Flatpickr Fin
     ✅ YA NO BLOQUEA POR SER DÍA ANTERIOR
     ✅ SOLO VALIDA QUE SEA >= FECHA INICIO
  ========================================================= */
  const fpEnd = flatpickr(fechaFinUI, {
    ...
    minDate: (fechaInicioHidden.value ? fechaInicioHidden.value : null),
  ...
  if (fechaInicioHidden.value && !fechaInicioUI.value) {
    fpStart.setDate(fechaInicioHidden.value, true);
  }

  if (fechaFinHidden.value && !fechaFinUI.value) {
    fpEnd.setDate(fechaFinHidden.value, true);
  }

  if (fechaInicioHidden.value) {
    fpEnd.set('minDate', fechaInicioHidden.value);
  }
  ...
  const modalDateChange = modalDateChangeEl
    ? new bootstrap.Modal(modalDateChangeEl, { backdrop: 'static' })
    : null;
  ...
  if (btnSendDateChange) {
    btnSendDateChange.addEventListener('click', function() {
      ...
      if (!endDb) {
        alert('Debes seleccionar la nueva fecha fin solicitada.');
        return;
      }

      if (!reason) {
        alert('Debes escribir el motivo del cambio de fecha.');
        return;
      }
  ...
  /* =========================================================
     ...
     ✅ YA PERMITE FECHA INICIO SERIE ANTERIOR
  ========================================================= */
  (function initRecurrenceUi() {
    ...
    recStartTimeEl.addEventListener('input', () => {
      ensureRecurrenceHiddenDates();
      updatePreview();
    });

    recEndTimeEl.addEventListener('input', () => {
      ensureRecurrenceHiddenDates();
      updatePreview();
    });
    ...
    form.addEventListener('submit', function(e) {
      ...
      const startDay = parseYmd(recStartDateEl.value);
      if (!startDay) {
        alert('Debes seleccionar la fecha inicio serie.');
        e.preventDefault();
        return;
      }
      ...
      if (!st || !et) {
        alert('Debes seleccionar la hora inicio y hora fin.');
        e.preventDefault();
        return;
      }
